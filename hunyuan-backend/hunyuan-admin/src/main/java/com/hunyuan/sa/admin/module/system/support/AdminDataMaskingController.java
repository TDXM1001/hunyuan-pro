...
import com.hunyuan.sa.base.module.support.datamasking.DataMasking;
import com.hunyuan.sa.base.module.support.datamasking.DataMaskingTypeEnum;
...
public class AdminDataMaskingController extends SupportBaseController {

    /**
     * 保留原有 URL，避免已部署的前端页面因命名治理产生兼容性问题。
     */
    @Operation(summary = "查询数据脱敏验证数据")
    @GetMapping("/dataMasking/demo/query")
    @SaCheckPermission("support:protect:dataMasking:query")
    public ResponseDTO<List<DataVO>> query() {
        ...
        for (int i = 0; i < RandomUtil.randomInt(10, 16); i++) {
            ...
            data.setIdCard("410" + RandomUtil.randomNumbers(3) + RandomUtil.randomInt(1980, 2010)
                    + RandomUtil.randomInt(10, 12) + RandomUtil.randomInt(10, 30)
                    + RandomUtil.randomNumbers(4));
            data.setAddress(RandomUtil.randomBoolean()
                    ? "河南省洛阳市洛龙区一零二四大街1024号"
                    : "河南省郑州市高新区六边形大街六边形大楼");
            data.setPassword(RandomUtil.randomString(10));
            data.setEmail(RandomUtil.randomString(RandomUtil.randomInt(6, 10))
                    + "@" + RandomUtil.randomString(2) + ".com");
            data.setCarLicense("豫" + RandomStringUtils.randomAlphabetic(1).toUpperCase()
