package com.hunyuan.sa.admin.module.platform.file;

import com.hunyuan.sa.base.common.domain.PageResult;
import com.hunyuan.sa.base.common.domain.RequestUser;
import com.hunyuan.sa.base.common.domain.ResponseDTO;
import com.hunyuan.sa.admin.module.platform.file.application.PlatformFileApplicationService;
import com.hunyuan.sa.admin.module.platform.file.domain.form.FileQueryForm;
import com.hunyuan.sa.admin.module.platform.file.domain.vo.FileDownloadVO;
import com.hunyuan.sa.admin.module.platform.file.domain.vo.FileMetadataVO;
import com.hunyuan.sa.admin.module.platform.file.domain.vo.FileUploadVO;
import com.hunyuan.sa.admin.module.platform.file.domain.vo.FileVO;
import com.hunyuan.sa.admin.module.platform.file.service.FileService;
...
class PlatformFileApplicationServiceTest {
    ...
    @Test
    void mapsOrderedFileLookupToStableSummaries() {
        FileVO first = new FileVO();
        first.setFileId(8L);
        first.setFileKey("private/common/first.png");
        first.setFileName("first.png");
        FileVO second = new FileVO();
        second.setFileId(9L);
        second.setFileKey("private/common/second.png");
        second.setFileName("second.png");
        List<String> keys = List.of(first.getFileKey(), second.getFileKey());
        when(fileService.getFileList(keys)).thenReturn(List.of(first, second));

        List<PlatformFileSummary> result = service.getFiles(keys);

        assertThat(result).extracting(PlatformFileSummary::getFileKey)
                .containsExactly(first.getFileKey(), second.getFileKey());
        verify(fileService).getFileList(keys);
    }
