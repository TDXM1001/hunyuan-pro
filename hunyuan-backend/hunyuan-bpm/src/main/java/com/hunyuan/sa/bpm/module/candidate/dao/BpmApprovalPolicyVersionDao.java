...
public interface BpmApprovalPolicyVersionDao extends BaseMapper<BpmApprovalPolicyVersionEntity> {
    ...
    @Update("UPDATE t_bpm_approval_policy_version SET policy_name=#{policyName}, description=#{description}, "
    ...
    @Update("DELETE FROM t_bpm_approval_policy_version WHERE policy_key=#{policyKey} AND policy_version=#{policyVersion} "
            + "AND lifecycle_state='DRAFT' AND catalog_revision=#{expectedRevision}")
