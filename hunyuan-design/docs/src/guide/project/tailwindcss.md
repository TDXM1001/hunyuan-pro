...
应用和包统一复用 `@hunyuan/vite-config`，并由该配置接入 `@tailwindcss/vite`。Tailwind 的扫描范围则统一在 `@hunyuan/tailwind-config` 对应的 `internal/tailwind-config/src/theme.css` 中维护。
...
当组件样式中使用 `@apply` 时，会自动注入 `@reference "@hunyuan/tailwind-config/theme"`，一般不需要手动补充。
