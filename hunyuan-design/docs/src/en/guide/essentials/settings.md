...
# API URL
VITE_GLOB_API_URL=https://localhost:5320/api
...
```ts
window._HUNYUAN_ADMIN_PRO_APP_CONF_ = {
  VITE_GLOB_API_URL: 'https://localhost:5320/api',
};
Object.freeze(window._HUNYUAN_ADMIN_PRO_APP_CONF_);
Object.defineProperty(window, '_HUNYUAN_ADMIN_PRO_APP_CONF_', {
  configurable: false,
  writable: false,
});
```
...
To access the variables inside `_app-config-{version}-{hash}.js`, you need to use the `useAppConfig` method provided by `@hunyuan/hooks`.
...
- First, add the variable that needs to be dynamically configurable in the `.env` file or the corresponding development environment configuration file. The variable must start with `VITE_GLOB_*`, for example:
  ...
  ```bash
  VITE_GLOB_OTHER_API_URL=https://localhost:5320/other-api
  ```
...
- In `packages/effects/hooks/src/use-app-config.ts`, add the corresponding configuration item, such as:
  ...
  export function useAppConfig(
    ...
    // In production environment, directly use the window._HUNYUAN_ADMIN_PRO_APP_CONF_ global variable
    const config = isProduction
      ? window._HUNYUAN_ADMIN_PRO_APP_CONF_
      : (env as HunyuanAdminProAppConfigRaw);
...
import { defineOverridesPreferences } from '@hunyuan/preferences';
...
const defaultPreferences: Preferences = {
  ...
  copyright: {
    companyName: 'Hunyuan',
    companySiteLink: 'https://github.com/hunyuan/hunyuan-design',
    date: '2024',
    enable: true,
    icp: '',
    icpLink: '',
