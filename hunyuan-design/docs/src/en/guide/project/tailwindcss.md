...
Apps and packages share `@hunyuan/vite-config`, which integrates `@tailwindcss/vite`. The Tailwind scan scope is managed centrally in `@hunyuan/tailwind-config`, backed by `internal/tailwind-config/src/theme.css`.
...
When component styles use `@apply`, `@reference "@hunyuan/tailwind-config/theme"` is injected automatically in most cases.
