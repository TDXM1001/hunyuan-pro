...
组件提供两种使用方式：通过 `items` 数据驱动（推荐），或通过子组件 `HunyuanDescriptionsItem` 声明列表项。两者可按需选择，`items` 优先级更高。:::
...
不传 `items` 时，可在默认插槽中使用 `HunyuanDescriptionsItem` 声明列表项，内容支持默认插槽或 `#content` 插槽自定义。
...
| title   | 自定义标题                         |
| extra   | 自定义标题右侧操作区域             |
| default | 放置 `HunyuanDescriptionsItem` 子组件 |

### DescriptionsItem

`items` 数组中的每一项，或子组件 `HunyuanDescriptionsItem` 的属性。
