...
`Alert` 与 `Modal` 的能力有部分重叠，但更适合临时确认、简单提示和轻量输入场景。复杂弹窗仍然建议使用 `Hunyuan Modal`。:::
...
<DemoPreview dir="demos/hunyuan-alert/alert" />

使用 `confirm` 创建带确认和取消按钮的提示框。

<DemoPreview dir="demos/hunyuan-alert/confirm" />

使用 `prompt` 创建可接收用户输入的提示框。

<DemoPreview dir="demos/hunyuan-alert/prompt" />
