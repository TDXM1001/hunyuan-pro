...
# Hunyuan Vxe Table 表格

`Hunyuan Vxe Table` 基于 [vxe-table](https://vxetable.cn/v4/#/grid/api?apiKey=grid) 和 `Hunyuan Form` 做了二次封装，用于构建带搜索表单的列表页面。
...
import { setupHunyuanVxeTable, useHunyuanVxeGrid } from '@hunyuan/plugins/vxe-table';
...
通过 `useHunyuanVxeGrid` 创建一个基础表格。

<DemoPreview dir="demos/hunyuan-vxe-table/basic" />

## 远程加载

通过配置 `proxyConfig.ajax.query` 实现远程数据加载。

<DemoPreview dir="demos/hunyuan-vxe-table/remote" />
...
<DemoPreview dir="demos/hunyuan-vxe-table/tree" />

## 固定列

固定列可选值为 `'left' | 'right' | '' | null`。

<DemoPreview dir="demos/hunyuan-vxe-table/fixed" />
...
<DemoPreview dir="demos/hunyuan-vxe-table/custom-cell" />

## 搜索表单

搜索区域底层使用的是 `Hunyuan Form`。启用搜索表单后，可以通过 `gridOptions.toolbarConfig.search = true` 在工具栏中显示搜索面板开关按钮。
...
<DemoPreview dir="demos/hunyuan-vxe-table/form" />

## 单元格编辑

通过设置 `editConfig.mode = 'cell'` 开启单元格编辑。

<DemoPreview dir="demos/hunyuan-vxe-table/edit-cell" />

## 行编辑

通过设置 `editConfig.mode = 'row'` 开启整行编辑。

<DemoPreview dir="demos/hunyuan-vxe-table/edit-row" />
...
<DemoPreview dir="demos/hunyuan-vxe-table/virtual" />

## API

`useHunyuanVxeGrid` 返回一个数组，第一个元素是表格组件，第二个元素是表格 API。

```vue
<script setup lang="ts">
import { useHunyuanVxeGrid } from '#/adapter/vxe-table';

const [Grid, gridApi] = useHunyuanVxeGrid({
  gridOptions: {},
  formOptions: {},
  gridEvents: {},
});
...
所有属性都通过 `useHunyuanVxeGrid` 的第一个参数传入。
...
| formOptions | 搜索表单配置 | `HunyuanFormProps` | - |
