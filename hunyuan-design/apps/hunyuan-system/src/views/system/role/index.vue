...
import {
  addRole,
  batchAddRoleEmployees,
  batchRemoveRoleEmployees,
  getRoleSelectedMenu,
  getRoleDataScopeList,
  listDataScopes,
  listRoles,
  queryCandidateRoleEmployees,
  queryRoleEmployees,
  removeRoleEmployee,
  updateRoleDataScopeList,
  updateRoleMenu,
} from '#/api/system/organization';

defineOptions({ name: 'SystemRoleIndex' });

interface RoleFormModel extends RoleAddForm {
  roleId?: number;
}

interface PermissionRow {
  actions: MenuSimpleTreeRecord[];
  depth: number;
  node: MenuSimpleTreeRecord;
}
...
const parentIdMap = computed(() => {
  ...
  function visit(nodes: MenuSimpleTreeRecord[]) {
    nodes.forEach((node) => {
      map.set(node.menuId, node.parentId ?? null);
      visit(node.children ?? []);
    });
  }
...
const permissionRows = computed<PermissionRow[]>(() => {
  ...
  function visit(nodes: MenuSimpleTreeRecord[], depth: number) {
    nodes
      .filter((node) => node.menuType !== 3)
      .forEach((node) => {
        const children = node.children ?? [];
        rows.push({
          actions: children.filter((child) => child.menuType === 3),
          depth,
          node,
        });
        visit(
          children.filter((child) => child.menuType !== 3),
          depth + 1,
        );
      });
  }
...
const employeeColumnsFactory = (): ColumnOption<EmployeeRecord>[] => [
  ...
  { prop: 'positionName', label: '职务', minWidth: 120 },
...
const candidateColumnsFactory = (): ColumnOption<EmployeeRecord>[] => [
  { type: 'selection', width: 50, align: 'center' },
  { prop: 'actualName', label: '员工姓名', minWidth: 120 },
  { prop: 'loginName', label: '登录账号', minWidth: 120 },
  { prop: 'departmentName', label: '部门', minWidth: 120 },
...
function collectDescendantIds(node: MenuSimpleTreeRecord): number[] {
  return [
    node.menuId,
...
function isNodeChecked(node: MenuSimpleTreeRecord) {
  return selectedMenuIds.value.includes(node.menuId);
}

function isNodeIndeterminate(node: MenuSimpleTreeRecord) {
  const descendantIds = collectDescendantIds(node).filter(
    (menuId) => menuId !== node.menuId,
  );
...
function handleNodeCheck(node: MenuSimpleTreeRecord, checked: boolean) {
  const selected = new Set(selectedMenuIds.value);

  if (checked) {
    collectDescendantIds(node).forEach((menuId) => selected.add(menuId));
    collectAncestorIds(node.menuId).forEach((menuId) => selected.add(menuId));
  } else {
...
async function loadRolePermissions(roleId: number) {
  ...
  try {
    const result = await getRoleSelectedMenu(roleId);
    menuTreeList.value = result.menuTreeList ?? [];
    selectedMenuIds.value = normalizeSelectedMenuIds(result.selectedMenuId ?? []);
  } finally {
...
async function loadRoleDataScopes(roleId: number) {
  ...
  try {
    const [scopeList, selectedList] = await Promise.all([
      dataScopes.value.length > 0 ? dataScopes.value : listDataScopes(),
      getRoleDataScopeList(roleId),
    ]);
    ...
    resetRoleDataScopeValues(
      Object.fromEntries(
        dataScopes.value.map((scope) => [
          scope.dataScopeType,
          selectedMap.get(scope.dataScopeType) ?? scope.viewTypeList[0]?.viewType ?? 0,
        ]),
      ),
    );
  } finally {
...
async function loadRoleEmployees(roleId: number) {
  ...
  try {
    const result = await queryRoleEmployees({
...
async function loadCandidateEmployees() {
  ...
  try {
    const result = await queryCandidateRoleEmployees({
...
async function handleSubmit() {
  ...
  try {
    await addRole(formData as RoleAddForm);
...
async function handleSavePermissions() {
  ...
  try {
    await updateRoleMenu({
      menuIdList: normalizeSelectedMenuIds(selectedMenuIds.value),
      roleId: activeRoleId.value,
    });
...
async function handleSaveDataScopes() {
  ...
  try {
    await updateRoleDataScopeList({
      dataScopeItemList: dataScopes.value.map((scope) => ({
        dataScopeType: scope.dataScopeType,
        viewType:
          roleDataScopeValues[scope.dataScopeType] ??
          scope.viewTypeList[0]?.viewType ??
          0,
      })),
      roleId: activeRoleId.value,
    });
...
function handleEmployeeSelectionChange(rows: EmployeeRecord[]) {
  selectedEmployeeIds.value = rows.map((row) => row.employeeId);
}

function handleCandidateSelectionChange(rows: EmployeeRecord[]) {
...
async function handleAddRoleEmployees() {
  ...
  await batchAddRoleEmployees({
    employeeIdList: selectedCandidateIds.value,
    roleId: activeRoleId.value,
  });
...
async function handleBatchRemoveRoleEmployees() {
  ...
  await batchRemoveRoleEmployees({
      ...
      <ElCard class="role-page__permission-card" shadow="never">
        <div class="role-page__permission-header">
          <div>
            ...
            <div class="role-page__permission-subtitle">
              {{ currentRole?.remark || currentRole?.roleCode || '选择左侧角色后配置权限' }}
            </div>
          </div>
          <ElButton
            v-if="activeTab !== 'employees'"
            :disabled="!activeRoleId"
            :loading="activeTab === 'dataScope' ? dataScopeSaving : permissionSaving"
            type="primary"
        ...
        <ElTabs v-model="activeTab" class="role-page__tabs">
          <ElTabPane label="角色-功能权限" name="permissions">
            <div v-loading="permissionLoading" class="role-page__permission-matrix">
              ...
              <ElScrollbar class="role-page__matrix-scrollbar">
                ...
                >
                  ...
                  <div class="role-page__matrix-actions">
                    ...
                    <span v-if="row.actions.length === 0" class="role-page__matrix-muted">
                      菜单访问
                    </span>
                  </div>
                </div>
          ...
          <ElTabPane label="角色-数据范围" name="dataScope">
            <div v-loading="dataScopeLoading" class="role-page__data-scope-panel">
              <div
              ...
              >
                <div class="role-page__data-scope-info">
                  ...
                  <div class="role-page__data-scope-desc">
                    {{ scope.dataScopeTypeDesc }}
                  </div>
                </div>
                <ElRadioGroup v-model="roleDataScopeValues[scope.dataScopeType]">
                  <ElRadioButton
                    v-for="viewType in scope.viewTypeList"
              ...
              <ArtTablePanel>
                ...
                >
                  <template #left>
                    <div class="role-page__employee-toolbar">
                      ...
                      <ElButton @click="openEmployeeDialog">添加员工</ElButton>
                      <ElButton
                ...
                >
                  <template #actions="{ row }">
                    ...
                    >
                      <template #reference>
                        <ElButton link size="small" type="danger">移除</ElButton>
                      </template>
                    </ElPopconfirm>
                    </div>
                  </template>
