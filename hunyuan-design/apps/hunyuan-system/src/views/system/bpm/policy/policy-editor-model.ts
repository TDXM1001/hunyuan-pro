...
export function createRoleCandidateModel(): BpmPolicyVisualDraft {
  return { type: 'CANDIDATE', policyKey: '', policyName: '', schemaVersion: 2, policyVersion: 1, catalogRevision: 0, candidate: { resolverType: 'ROLE', resolutionPhase: 'ACTIVATE', memberOrder: 'SELECTION_ORDER', emptyCandidatePolicy: 'BLOCK', selfApprovalPolicy: 'BLOCK' } };
}

export function createPolicyModel(type: BpmPolicyVisualDraft['type']): BpmPolicyVisualDraft {
  const base = { type, policyKey: '', policyName: '', schemaVersion: 2 as const, policyVersion: 1, catalogRevision: 0 };
