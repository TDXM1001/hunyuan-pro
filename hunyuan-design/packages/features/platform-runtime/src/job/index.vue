...
import { usePlatformRuntimeRequestClient } from '../dependencies';
const requestClient = usePlatformRuntimeRequestClient();
...
async function handleSubmit() {
  ...
  if (dialogMode.value === 'add') {
    await addJob(requestClient, formData);
    ElMessage.success('新增任务成功');
  } else {
    await updateJob(requestClient, formData);
...
async function handleToggleEnabled(row: JobRecord) {
  try {
    ...
    await updateJobEnabled(requestClient, {
      enabledFlag: !row.enabledFlag,
      jobId: row.jobId,
    });
...
async function handleExecute(row: JobRecord) {
  ...
  try {
    ...
    await executeJob(requestClient, {
      jobId: row.jobId,
      param: row.param,
    });
...
async function handleDelete(row: JobRecord) {
  try {
    ...
    await deleteJob(requestClient, row.jobId);
