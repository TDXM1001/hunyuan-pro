...
defineOptions({ name: 'AccessMenuManagement' });

interface MenuTreeRow extends AccessMenuRecord {
  children?: MenuTreeRow[];
  level: number;
}

interface MenuFormModel extends AccessMenuCommand {
  menuId?: number;
}
...
const rawTree = ref<AccessMenuTreeRecord[]>([]);
...
const dialogTitle = computed(() => {
  ...
  return addParentName.value
    ? `新增${addParentName.value}下级菜单`
    : '新增顶级菜单';
});

function getMenuTypeLabel(menuType: number) {
  return (
    MENU_TYPE_OPTIONS.find((item) => item.value === menuType)?.label ?? '未知'
  );
}
...
async function loadData() {
  loading.value = true;
  try {
    rawTree.value = await accessClient.listMenuTree(false);
  } finally {
    loading.value = false;
  }
}
...
async function handleDelete(row: MenuTreeRow) {
  try {
    ...
    await accessClient.deleteMenus([row.menuId]);
...
async function handleSubmit() {
  ...
  if (dialogMode.value === 'add') {
    ...
    await accessClient.updateMenu(formData as AccessMenuUpdateCommand);
    ...
    <div class="menu-page">
      ...
      <ElCard class="menu-page__table-card" shadow="never">
        <ArtTablePanel>
          ...
          >
            <template #left>
              <AccessControl :codes="['access.menu.create']" type="code">
          ...
          >
            ...
            <template #status="{ row }">
              <ElSpace :size="6">
                <ElTag
                  :type="row.visibleFlag ? 'success' : 'info'"
                  effect="plain"
                  size="small"
                >
            ...
            <template #actions="{ row }">
              <ElSpace class="menu-page__actions">
                ...
                <AccessControl :codes="['access.menu.update']" type="code">
                  <ElButton
                    link
                    size="small"
                    type="primary"
                ...
                <AccessControl :codes="['access.menu.delete']" type="code">
                  <ElButton
                    link
                    size="small"
                    type="danger"
        ...
        <div class="menu-page__form-grid">
          ...
          <ElFormItem label="组件路径" prop="component">
            <ElInput
              ...
              placeholder="例如 /system/menu/menu-list.vue"
            />
          </ElFormItem>
          <ElFormItem label="前端权限码" prop="webPerms">
            <ElInput
              v-model="formData.webPerms"
              placeholder="例如 access.menu.create"
            />
          </ElFormItem>
          <ElFormItem label="后端权限码" prop="apiPerms">
            <ElInput
              v-model="formData.apiPerms"
              placeholder="例如 access.menu.create"
            />
          </ElFormItem>
          ...
          <ElFormItem label="关联菜单ID" prop="contextMenuId">
            <ElInputNumber
              v-model="formData.contextMenuId"
              :min="0"
              style="width: 100%"
            />
          </ElFormItem>
