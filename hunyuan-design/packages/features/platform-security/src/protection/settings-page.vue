...
import type { Level3ProtectConfigFormModel } from './client';
...
import { usePlatformSecurityRequestClient } from '../dependencies';

const requestClient = usePlatformSecurityRequestClient();
...
async function loadConfig() {
  loading.value = true;
  try {
    const result = await queryLevel3ProtectConfig(requestClient);
...
async function handleSave() {
  ...
  saving.value = true;
  try {
    await updateLevel3ProtectConfig(requestClient, formData);
