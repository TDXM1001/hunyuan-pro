...
import { usePlatformNotificationRequestClient } from '../dependencies';

const requestClient = usePlatformNotificationRequestClient();
...
async function handleSubmit() {
  ...
  if (dialogMode.value === 'add') {
    await addSmsTemplate(requestClient, formData as SmsTemplateAddForm);
    ElMessage.success('新增短信模板成功');
  } else {
    await updateSmsTemplate(requestClient, formData as SmsTemplateUpdateForm);
...
async function handleToggleDisabled(row: SmsTemplateRecord) {
  try {
    ...
    await updateSmsTemplateDisabled(
      requestClient,
      row.templateCode,
      !row.disableFlag,
    );
