...
9. 明确接受表单设计器需要第三方依赖，并将其视为一期范围内的必要依赖，而不是可选优化
...
- 表单设计器第三方依赖属于 P0 in scope，不走自研设计器路线
...
- 表单设计器在当前仓不是“是否要第三方依赖”的选择题
- 它本身就需要第三方依赖支持
- 本次设计要解决的是“如何把第三方依赖安全封装进 adapter”，而不是“是否完全不用第三方依赖”
...
- 一期必须引入第三方设计器内核
- 可参考参考仓当前使用的 `@form-create/designer` 这一类能力源
- 参考仓中同时出现了渲染层依赖线索，但不机械锁定为其当前包名和版本

具体包名、桥接方式和最终版本以当前 `Vue 3 + Element Plus + 项目构建链` 兼容性验证后再落定，不在设计稿中机械继承参考仓版本。

### 依赖风险控制

- 所有依赖只进 `@hunyuan/system`，不扩散到全仓公共包
- 所有 adapter 必须处理销毁和全局实例清理
- 表单设计器依赖选型必须先做一次兼容性 spike，确认在当前技术栈下可稳定渲染、可导出快照、可通过 typecheck
...
2. 先做表单设计器第三方依赖兼容性 spike，确认最小可行依赖组合
3. 优先补齐轻模块管理页：分类、监听器
4. 再补齐定义、实例、任务的统一右侧详情抽屉
5. 再收口表单列表和模型列表的页面交互
6. 落表单设计页壳层与 `BpmFormDesignerAdapter`
7. 落流程设计页壳层与 `BpmProcessDesignerAdapter`
8. 最后做设计器联调、抽屉回归、类型校验与路由回归
