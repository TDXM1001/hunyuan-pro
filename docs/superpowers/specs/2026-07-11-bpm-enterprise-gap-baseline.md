...
| 任务/执行监听器 | M6 已形成版本化事件订阅、冻结连接器/签名/重试事实，不开放 class/EL | 参考可登记 execution/task listener | `HARDEN` 已落地 | M5/M6 |
...
| 命令、通知、回调记录 | M6 已在既有幂等、重试、死信与人工补偿资产上冻结订阅投递事实 | 参考也有消息和触发器 | `KEEP+REFACTOR` 已落地，M7 继续运营治理 | M4/M5/M6/M7 |
| 业务结果事件 | M6 已按固定事件协议和已发布订阅扇出可靠投递 | 参考有 OA 请假和监听器接入 | `KEEP+REFACTOR` 已落地 | M6 |
| 业务接入与审批详情 | M3 负责版本化业务契约与审批对象；M6 已完成来源登记、类型化绑定、固定开放协议和可靠订阅 | 参考支持自定义创建/详情路径 | `REFACTOR` 已落地 | M3/M6 |
| 内外任务入口 | M6 开放任务详情/动作协议已复用 M4 统一命令、员工映射和服务端字段权限 | 成熟平台支持嵌入和开放任务 API | `ADAPT` 已落地 | M4/M6 |
...
DONE  M6 配置化业务接入；版本化控制面、固定开放协议和可靠事件订阅已关闭
P3  M7 运营治理
P4  M8 定义演进与受限迁移
```
