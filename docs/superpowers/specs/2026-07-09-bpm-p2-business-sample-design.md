...
- 前端合同测试只固定样板 API 与入口，不在本轮建设复杂业务页面。
...
3. 如果 `callbackFailFlag = true`，清空该标记并返回失败，模拟业务侧短暂不可用。
4. 如果 `callback_event_id` 已等于 `context.eventId`，返回成功摘要，不重复回写。
5. 从 `requestPayloadJson` 解析 `BpmBusinessResultEvent`。
6. `resultState = 1` 时写为已通过并设置 `approvedAt`。
7. `resultState = 2` 时写为已拒绝并设置 `rejectedAt`。
8. 其他结果状态返回失败，失败原因写明未知审批结果。
...
如果需要可视化验收，再新增一个安静的样板页：

- `hunyuan-design/apps/hunyuan-system/src/views/system/bpm/sample/expense-sample.vue`

页面只提供创建样板、发起流程、设置下一次回调失败、查看实例 ID 的最小操作，不做完整费用列表、报销附件或财务审核。
...
4. BPM 创建实例和命令记录。
5. 审批结束后发布 `BpmBusinessResultEvent`。
6. P2.3 回调记录进入待处理。
7. 回调执行器找到 `BpmSampleExpenseCallbackHandler`。
8. handler 将样板状态写为已通过或已拒绝。
9. 回调记录写为成功。
10. 管理员可在实例 trace 和回调记录列表看到成功回调。
...
- 如新增样板页，则 `bpm-modules.test.ts` 固定页面路径和关键 API 调用。
...
- 前端是否只做 API 合同，还是增加最小样板页用于浏览器验收。
