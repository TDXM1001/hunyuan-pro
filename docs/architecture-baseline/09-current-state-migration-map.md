...
目标更新（2026-07-24）：项目已经明确以“可复用管理后台底座”作为当前产品交付目标。应用装配、模块注册、稳定路由、平台 feature 归位和 `hunyuan-base` 收缩因此属于真实产品需求，不是为了目录整齐进行的无目标搬迁。A4.5 之后先执行 [27-foundation-structure-closeout-blueprint.md](27-foundation-structure-closeout-blueprint.md) 的 F0-F6，关闭底座结构后再进入真实业务纵切。
