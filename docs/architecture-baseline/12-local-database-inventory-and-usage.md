...
| --- | --- | --- | --- | --- | --- |
| `hunyuan` | 开发运行库 | 日常开发、接口联调、浏览器验收和保留开发数据 | 是 | 48 张表，Flyway `3.67.0` | 不得由隔离测试重建或清空；迁移前先备份和预检 |
| `hunyuan_a1_it` | 集成测试隔离库 | 验证 Flyway、管理员 bootstrap、Redis 隔离和后端集成契约 | 否 | 48 张表，Flyway `3.67.0` | 只能通过 `_it` 测试入口使用；不能替代开发库 |
...
- Flyway 已成功记录 `3.66.0`、`3.66.1` 和 `3.67.0`。
- A2 组织目录模块开关为 `true`。
- 组织目录菜单 `perms_type=1`，4 个 `organization.department.*` 能力已落库。
- A2.1 已删除旧 `/organization/department` 菜单、`system:department:*` 菜单及其角色菜单关联；旧部门菜单和旧授权均为 0。
...
2026-07-21 先完整验证该库可从既有基线升级至 `3.66.1`，随后通过 A2.1 隔离测试迁移到 `3.67.0`。迁移后已验证组织目录菜单及 4 个能力记录均为 `perms_type=1`，旧部门菜单、旧权限码和旧角色菜单关联均为 0。
