...
| 商品、分类示例                      | `business.goods`、`business.category`                                             | 无                           | RETIRE       | A3.4 P1 已关闭                         |
| OA 企业、员工关联、银行、发票         | 历史 `business.oa` 主数据子域                                                      | 无                          | RETIRE       | A3.4 P2 已关闭                          |
| OA 通知与公告                         | 历史 `business.oa.notice`                                                         | 无                           | RETIRE       | A3.4 P3 已关闭，未迁移到 `message`      |
...
状态：**2026-07-22 P0-P5 已完成，A3.3 已正式关闭。** 详细冻结账本见
...
状态：**2026-07-23 P1 商品与分类历史示例簇、P2 企业/员工关联/银行/发票、P3 OA
通知均已完成备份、迁移、实现删除和运行态验收。OA 通知未迁移到 `message`，
A3.4 业务示例与 OA 退役范围为 `RETIRE_CLOSED`；工程 Demo 按私有资产边界保留。**
详细账本见
[18-a3-4-platform-support-and-example-retirement.md](18-a3-4-platform-support-and-example-retirement.md)、
[19-a3-4-oa-adoption-audit.md](19-a3-4-oa-adoption-audit.md) 和
...
- 对商品、分类、OA 和其他历史示例模块做真实采用审计并形成处置决定。
- 对确认无消费者的页面、菜单、API、表和代码按独立迁移退役。
- 工程验收 Demo、数据脱敏验证和 backend-mock 不按业务示例批量删除，分别执行生产隔离、
  能力改名或开发环境限制。
...
当前状态（2026-07-22）：**A3.1、A3.2、A3.3 已正式关闭。**
...
A3.2 的所有权、入口、消费者、能力码草案、分批计划和关闭条件见 [16-a3-2-access-contract-and-consumer-freeze.md](16-a3-2-access-contract-and-consumer-freeze.md)。P1 第一批已建立 `AccessAuthorizationFacade`，登录不再直接依赖角色 Service/DAO；P2 已完成角色分配、数据范围查询与管理、组织范围、角色生命周期、能力授权与查询、菜单目录、角色成员和菜单 owner 查询边界，并由 access 统一拥有数据范围类型枚举。P3 新增 `V3.70.0`，建立 15 个 `access.*` 稳定能力节点，为平台管理员和既有旧权限角色迁移授权，保留旧权限至 P5；同时对 `t_role_menu`、`t_role_data_scope` 的既有唯一约束增加去重和幂等补建守卫。新增一致性测试严格对账 5 个稳定 Controller、冻结能力集合和 Flyway 能力目录。完整 Maven reactor 中 `hunyuan-base` 12 个测试、`hunyuan-admin` 152 个测试均无失败或错误，ArchUnit 24 条通过；全新 `hunyuan_a3_2_p3_it` 隔离库的 Flyway、管理员初始化和 Redis 隔离 3 项真实集成测试全部通过，最终迁移版本为 `3.70.0`。

P4 已建立 `@hunyuan/feature-access`，由 feature 拥有角色管理、角色能力授权、数据范围、角色成员和菜单管理页面、稳定客户端、契约类型、路由元数据及 15 个能力码。应用内原角色、菜单路由页面已收缩为客户端注入和 feature 渲染薄入口，前端请求统一切换到 `/admin/v1/access/*`；旧 `api/system/menu.ts` 已删除，`api/system/organization.ts` 只保留岗位兼容 API。新增 access 客户端契约测试并更新应用边界测试，3 个测试文件共 21 项通过；`@hunyuan/system` 类型检查和改动文件 ESLint 均通过。

P5 已删除六个旧角色、菜单和数据范围 Controller 及无剩余生产消费者的兼容 DTO、Service 与枚举；`V3.71.0` 已从开发库删除旧角色授权关系和旧权限节点。开发库 `hunyuan` 已迁移至 Flyway `3.71.0`，稳定 access 能力 15 个、旧权限节点 0 个、旧授权关系 0 条，`platform_admin` 拥有全部 15 个稳定授权。旧 `/role/*`、`/menu/*`、`/dataScope/*` 代表路径均返回 HTTP 404，OpenAPI 中稳定 access 路由 13 条、旧路由 0 条，干净构建产物中旧 Controller 字节码为 0。

角色管理与菜单管理页面已完成真实浏览器验收，页面数据和关键操作区正常，控制台无错误。开发态重复 Vue 运行时问题已通过 Vite `resolve.dedupe` 统一 `vue` 与 `element-plus` 解决。仓库内生产消费者已归零，残余旧名称或路径仅属于负向测试、历史迁移、关闭文档及非目标 demo/mock；仓库外消费者按“从未生产、未开放正式外部集成”判定为不适用（N/A）。P5 最终回归中，前端 21 项测试、类型检查与 ESLint 均通过；完整 Maven reactor 中 `hunyuan-base` 12 项、`hunyuan-admin` 140 项均无失败或错误，3 个外部环境集成测试按配置跳过，ArchUnit 18 项通过。最终 codebase-memory 全量索引 `E-my-project-hunyuan-pro-a3-2-p5-closed-20260722` 已持久化。完整关闭证据见 [16-a3-2-access-contract-and-consumer-freeze.md](16-a3-2-access-contract-and-consumer-freeze.md)。

**A3.2 已正式关闭，下一步进入 A3.3 岗位目录迁移。**

A3.3 已完成 P0 契约冻结、P1/P2 稳定岗位边界与引用保护、P2 悬空引用修复、P3 数据权限
迁移、P4 前端纵切和 P5 兼容退役。`@hunyuan/feature-organization/position-directory`
拥有岗位页面、契约、稳定客户端和依赖注入键；岗位应用页面已收缩为薄入口，员工岗位
选项 provider 已切换到稳定岗位客户端。旧前端 `api/system/organization.ts` 及其测试
已删除，活跃前端不再消费旧 `/position/*`。

P5 已删除旧 `system.position` Controller、Service、DAO、Manager、DTO/Entity 和 Mapper；
`V3.73.0` 已从开发库删除旧岗位授权关系和旧 `system:position:*` 权限节点。开发库
`hunyuan` 最终版本为 `3.73.0`，有效岗位 4 个、非空员工岗位引用 9 条、悬空引用 0 条、
稳定岗位能力及平台管理员授权各 4 条、旧岗位权限节点与授权均为 0。旧 `/position/*`
代表路径严格返回 HTTP 404，OpenAPI 中稳定岗位路径 2 条、旧路径 0 条，干净构建产物中
旧岗位兼容字节码为 0。

最终回归中，完整 Maven reactor 的 `hunyuan-base` 12 项、`hunyuan-admin` 153 项测试
无失败或错误，3 项按配置跳过，ArchUnit 18 项通过；全新隔离库的 Flyway、管理员初始化
和 Redis 隔离测试全部通过。前端 61 个测试文件、426 项测试全部通过，类型检查、生产构建、
ESLint 和严格 UTF-8 检查通过；真实浏览器确认岗位页和员工编辑岗位下拉均显示 4 个稳定岗位，
刷新后控制台无错误。最终 codebase-memory 全量索引
`E-my-project-hunyuan-pro-a3-3-p5-closed-20260722` 已建立，状态为 `ready`，包含
16,117 个节点和 40,526 条关系；旧 `PositionController`、`PositionDao` 图节点均为 0，
